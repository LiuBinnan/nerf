...
def visualize_cameras(colored_camera_dicts, sphere_radius, camera_size=0.1, geometry_file=None, geometry_type='mesh'):
    ...
    pcd = o3d.io.read_point_cloud("C:\\Users\\LiuBinnan\\Desktop\\code\\nerf\\data\\nerf_example_data\\nerf_llff_data\\fern\\points.ply")
    ...
    for color, camera_dict in colored_camera_dicts:
        ...
        for img_name in sorted(camera_dict.keys()):
            ...
            cnt += 1
            cam_coord_frame = o3d.geometry.TriangleMesh.create_coordinate_frame(0.05, origin=C2W[:3, 3])
            cam_coord_frame.rotate(C2W[:3, :3])
            things_to_draw.append(cam_coord_frame)
...
if __name__ == '__main__':
    ...
    base_dir = 'C:\\Users\\LiuBinnan\\Desktop\\code\\nerf\\data\\nerf_example_data\\nerf_llff_data\\fern'

    sphere_radius = 1.
    train_cam_dict = json.load(open(os.path.join(base_dir, 'cam.json')))
    avg_cam_dict = json.load(open(os.path.join(base_dir, 'avg_cam.json')))
    render_cam_dict = json.load(open(os.path.join(base_dir, 'render_cam.json')))
    camera_size = 0.1
